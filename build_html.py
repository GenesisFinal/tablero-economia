...
def build_index_html():
    print("Building index.html with Sidebar Navigation & 2-Row Top Menu Layout...")
...
<head>
  ...
  <style>
    ...
    .sidebar-item.active {{
      background: linear-gradient(90deg, rgba(226, 0, 57, 0.2) 0%, rgba(226, 0, 57, 0.05) 100%);
      border-left: 3px solid #E20039;
      color: #FFFFFF;
      font-weight: 700;
    }}
...
<body class="min-h-screen flex flex-col selection:bg-brand-red selection:text-white">
  ...
  <!-- TOP HEADER -->
  <header class="sticky top-0 z-40 bg-[#0F172A]/95 dark:bg-[#0F172A]/95 light:bg-white/95 backdrop-blur-md border-b border-[#334155]/60 light:border-slate-200 transition-colors">
    <div class="max-w-[1900px] mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
      
      <!-- Left: Mobile Menu Toggle & Brand Logo -->
      <div class="flex items-center gap-3 shrink-0">
        <button onclick="toggleMobileSidebar()" class="lg:hidden p-2 rounded-xl bg-[#1E293B] light:bg-slate-100 text-slate-300 light:text-slate-700 hover:text-white border border-[#334155] light:border-slate-300">
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="hidden sm:flex items-center p-1 rounded-xl bg-[#1E293B] light:bg-slate-100 border border-[#334155] light:border-slate-300 text-xs font-semibold">
          <button 
          ...
          <button 
            ...
            class="px-2.5 py-1 rounded-lg transition-all flex items-center gap-1.5 text-slate-400 light:text-slate-600 hover:text-white light:hover:text-black"
        ...
        <button onclick="toggleTheme()" id="theme-toggle-btn" title="Cambiar Tema (Oscuro / Claro)" class="p-2 w-9 h-9 rounded-xl bg-[#1E293B] light:bg-slate-100 text-slate-300 light:text-slate-700 hover:text-white light:hover:text-black border border-[#334155] light:border-slate-300 flex items-center justify-center text-sm transition-colors">
  ...
  <section class="bg-[#0F172A]/60 light:bg-slate-100/80 border-b border-[#334155]/40 light:border-slate-200 py-3 transition-colors">
  ...
  <nav id="top-categories-grid-nav" class="hidden bg-[#0B1120]/95 light:bg-[#F8FAFC]/95 border-b border-[#334155]/50 light:border-slate-200 py-3 transition-colors">
    <div class="max-w-[1900px] mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-[11px] font-bold uppercase tracking-wider text-slate-400 light:text-slate-500 mb-2 flex items-center justify-between">
        <span>Categorías Macroeconómicas</span>
        <span class="text-[10px] text-brand-red font-mono">12 Secciones</span>
      </div>
      <div class="flex flex-wrap gap-2 text-xs font-semibold" id="top-grid-tabs-container">
        <!-- Rendered dynamically (wraps in 2 neat rows without horizontal scroll) -->
      </div>
    </div>
  </nav>
  ...
  <div class="max-w-[1900px] mx-auto px-4 sm:px-6 lg:px-8 py-6 w-full flex-grow flex items-start gap-6">
    ...
    <aside id="left-sidebar" class="w-64 xl:w-72 shrink-0 sticky top-20 flex flex-col gap-4 max-h-[calc(100vh-100px)] overflow-y-auto pr-1">
      ...
      <!-- Sidebar Box -->
      <div class="glass-card rounded-2xl p-3 border border-[#334155]/60 light:border-slate-200">
        
        <!-- Sidebar Header -->
        <div class="px-3 py-2 border-b border-[#334155]/50 light:border-slate-200 flex items-center justify-between mb-2">
          <span class="text-xs font-bold uppercase tracking-wider text-slate-300 light:text-slate-700 flex items-center gap-1.5">
            <i class="fas fa-layer-group text-brand-red"></i>
            <span>Categorías</span>
          </span>
          <span class="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-slate-800 light:bg-slate-200 text-slate-400 light:text-slate-600" id="sidebar-total-badge">88</span>
        </div>

        <!-- Sidebar Category Links List -->
        <div class="flex flex-col gap-1 text-xs" id="sidebar-category-list">
          <!-- Rendered dynamically -->
        </div>
      ...
      <!-- Quick Info Box -->
      <div class="glass-card rounded-2xl p-4 border border-[#334155]/50 light:border-slate-200 text-xs flex flex-col gap-2">
        <div class="flex items-center gap-2 text-emerald-400 font-bold">
          <span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
          <span>Verificación Automática</span>
        </div>
        <p class="text-[11px] text-slate-400 light:text-slate-600 leading-relaxed">
          Series auditadas 2 veces al día desde BCRA, INDEC y ArgentinaDatos.
        </p>
        <div class="text-[10px] text-slate-400 light:text-slate-500 font-mono mt-1 pt-2 border-t border-[#334155]/40 light:border-slate-200">
          Act: <span id="sidebar-update-time" class="text-slate-300 light:text-slate-700">...</span>
        </div>
      </div>
    ...
    <div id="mobile-sidebar-backdrop" class="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm hidden opacity-0 transition-opacity duration-200" onclick="toggleMobileSidebar()">
      <div id="mobile-sidebar-drawer" class="w-72 max-w-[85vw] h-full bg-[#0F172A] light:bg-white p-4 shadow-2xl flex flex-col gap-4 overflow-y-auto transform -translate-x-full transition-transform duration-250 ease-out" onclick="event.stopPropagation()">
        
        <div class="flex items-center justify-between pb-3 border-b border-slate-700 light:border-slate-200">
          <div class="flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-brand-red"></span>
            <span class="font-bold text-sm text-white light:text-black">Categorías Económicas</span>
          </div>
          <button onclick="toggleMobileSidebar()" class="p-1.5 rounded-lg text-slate-400 hover:text-white">
            <i class="fas fa-times"></i>
          </button>
        </div>
  ...
  <footer class="bg-[#0F172A] light:bg-slate-100 border-t border-[#334155]/60 light:border-slate-200 py-6 mt-12 transition-colors">
    <div class="max-w-[1900px] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400 light:text-slate-600">
      <div class="flex items-center gap-2">
  ...
  <script>
    ...
    function setNavLayout(mode) {{
      ...
      if (mode === 'topgrid') {{
        ...
        if (btnSidebar) {{
          btnSidebar.className = "px-2.5 py-1 rounded-lg transition-all flex items-center gap-1.5 text-slate-400 light:text-slate-600 hover:text-white light:hover:text-black";
        }}
      }} else {{
        // Sidebar Mode
        ...
        if (btnTopGrid) {{
          btnTopGrid.className = "px-2.5 py-1 rounded-lg transition-all flex items-center gap-1.5 text-slate-400 light:text-slate-600 hover:text-white light:hover:text-black";
        }}
      }}
    }}
    ...
    function renderSidebarNavigation() {{
      ...
      let html = `
        <button 
          ...
          id="sidebar-item-all"
          class="sidebar-item active w-full px-3 py-2 rounded-xl text-left transition-all flex items-center justify-between text-slate-300 light:text-slate-700 hover:bg-slate-800/60 light:hover:bg-slate-200/70"
        >
          <span class="flex items-center gap-2">
            <i class="fas fa-layer-group text-slate-400"></i>
            <span>Todas las Categorías</span>
          </span>
          <span class="text-[10px] px-2 py-0.5 rounded-full bg-slate-800 light:bg-slate-300 font-mono font-bold">${{totalCount}}</span>
        </button>
      `;
      ...
      categories.forEach(cat => {{
        ...
        html += `
          <button 
            ...
            class="sidebar-item w-full px-3 py-2 rounded-xl text-left transition-all flex items-center justify-between text-slate-400 light:text-slate-600 hover:text-white light:hover:text-black hover:bg-slate-800/60 light:hover:bg-slate-200/70"
          >
            <span class="flex items-center gap-2 truncate">
              <i class="fas ${{cat.icon}} text-brand-red w-4 text-center"></i>
              <span class="truncate">${{cat.name}}</span>
            </span>
            <span class="text-[10px] px-1.5 py-0.5 rounded-full bg-slate-800/80 light:bg-slate-200 font-mono font-bold shrink-0">${{count}}</span>
          </button>
        `;
      }});
    ...
    function renderTopGridNavigation() {{
      ...
      let html = `
        <button 
          ...
          class="top-tab-btn active px-3 py-1.5 rounded-xl border border-transparent transition-all flex items-center gap-1.5 text-slate-300 light:text-slate-700"
        >
      ...
      categories.forEach(cat => {{
        ...
        html += `
          <button 
            ...
            class="top-tab-btn px-3 py-1.5 rounded-xl border border-transparent transition-all flex items-center gap-1.5 text-slate-400 light:text-slate-600 hover:text-white light:hover:text-black bg-[#1E293B] light:bg-slate-200/80"
          >
            <i class="fas ${{cat.icon}} text-brand-red"></i>
            <span>${{cat.name}}</span>
            <span class="text-[10px] px-1.5 py-0.2 rounded-full bg-slate-800 light:bg-slate-300 font-mono font-bold">${{count}}</span>
          </button>
        `;
      }});
    ...
    function selectCategory(catId) {{
      ...
      document.querySelectorAll('.sidebar-item').forEach(btn => {{
        ...
        btn.classList.add('text-slate-400', 'light:text-slate-600');
      }});
      ...
      document.querySelectorAll('.top-tab-btn').forEach(btn => {{
        ...
        btn.classList.add('bg-[#1E293B]', 'light:bg-slate-200/80', 'text-slate-400', 'light:text-slate-600');
      }});
      const activeTop = document.getElementById(`top-grid-item-${{catId}}`);
      if (activeTop) {{
        activeTop.classList.add('active', 'bg-brand-red', 'text-white');
        activeTop.classList.remove('bg-[#1E293B]', 'light:bg-slate-200/80', 'text-slate-400', 'light:text-slate-600');
      }}
...
'''
    ...
    print(f"[SUCCESS] Wrote index.html with Sidebar & 2-Row Top Grid Layout ({len(html_content)} bytes)!")
